...
def GATTA_Net(input_shape, num_classes, learning_rate, graph, Merge):
    ...
    with graph.as_default():
        ...
        X_trans = tf.reshape(X, [-1, 28, 28, 1])
        
        conv1 = conv(X_trans, 3, 3, 16, 1, 1, name='conv1')
        norm1 = lrn(conv1, 2, 2e-05, 0.75, name='norm1')
        pool1 = max_pool(norm1, 2, 2, 2, 2, padding='VALID', name='pool1')

        # 2nd Layer: Conv (w ReLu)  -> Lrn -> Pool with 2 groups
        conv2 = conv(pool1, 3, 3, 16, 1, 1, name='conv2')
        norm2 = lrn(conv2, 2, 2e-05, 0.75, name='norm2')
        pool2 = max_pool(norm2, 2, 2, 2, 2, padding='VALID', name='pool2')

        flattened = tf.reshape(pool2, [-1, 7 * 7 * 16])

        fc1 = fc_layer(flattened, 7 * 7 * 16,  2048, name='fc1')

        lp = tf.placeholder(tf.float32, [1, 1, 2048*num_classes+num_classes], name='Local_p2')
        np = tf.placeholder(tf.float32, [1, None, 2048*num_classes+num_classes], name='Neig_p2')

        logits_ns, para = fc_layer_ns(fc1, lp, np, 2048, num_classes, elu=False, name='fc2_ns', name2='GAT2')
        # loss and optimizer
...
def GAT_layer(local_paras, neigh_paras, out_sz, name2, in_drop=0.0, coef_drop=0.0, residual=False, activation=tf.nn.elu):
    ...
    with tf.variable_scope(name2) as scope:
        ...
        if in_drop != 0.0:
            seq_neig = tf.nn.dropout(seq_neig, 1.0 - in_drop)
