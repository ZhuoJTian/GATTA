...
from Sample_parti_noiid2 import getAttr
import os
...
def buildClients(num, Users, UsersToClient, local_dist, local_dist_test, Me):
    # flag=0, D-SGD;  flag=1, ADMM
    learning_rate = 0.01
    num_input = 28  # image shape: 28*28
    num_input_channel = 1  # image channel: 1
    ...
    return Clients_GATTA(input_shape=[None, num_input*num_input, num_input_channel],
                   ...
                   learning_rate=learning_rate, Users=Users, UsersToClient=UsersToClient,
...
def GATTA_run_local_test(clients, adjm, global_vars, para_ns, ep):
    ...
    with open("./record1/local_GATNN2_mu7.txt", "a+") as f:
...
def GATTA(client):
    #### BEGIN TRAINING ####
    global_vars = [0] * CLIENT_NUMBER
    para_agnos = [0] * CLIENT_NUMBER
    para_ns = [0] * CLIENT_NUMBER
    vars = client.get_client_vars()
    vars_ag, index_ag = client.get_agnostic_vars()
    shape_ns = [1, 1, 2048*CATEGORIES+CATEGORIES]
    ...
    for ep in range(round):
        ...
        for client_id in tqdm(range(CLIENT_NUMBER), ascii=True):
            ...
            para_new[client_id] = client.get_ns_paras(local_old, neig_old)
            '''
            if ep==round-1:
                client.save_model(client_id)'''

        para_ns = deepcopy(para_new)
...
epoch_total = 800
epoch_local = 1
round=int(epoch_total/epoch_local)  #100 round
adj_matrix=np.loadtxt('adj_matrix.txt')

#### CREATE CLIENT AND LOAD DATASET ####
Users, UsersToClients, LocalDist, LocalDist_test, SamplesToClients, SamplesToClients_test, MAX_NUM = getAttr()
client = buildClients(CLIENT_NUMBER, Users, UsersToClients, local_dist=LocalDist, local_dist_test=LocalDist_test, Me=2)
GATTA(client)
